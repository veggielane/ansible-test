...
every playbook and role in this project run for real, end to end.

```
C:\FakeWindchill\Windchill\            <- windchill_home in inventories/lab
...
└── fakedb\oir\<container>\<rule>.xml  <- "loaded" rules end up here
...
| Rules are keyed by name inside a container | writes/overwrites `fakedb\oir\<container>\<name>.xml` |

Things to try (all from lesson 07):
...
- delete a file under `fakedb\oir\` to see that Ansible does *not* notice
